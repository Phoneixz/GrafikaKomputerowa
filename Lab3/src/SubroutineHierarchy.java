	 ...
	 private void rotatingPolygon(Graphics2D g2,double skala_x , double skala_y, double translate_x ,double translate_y) // polygon
		{ 
		 AffineTransform saveTransform = g2.getTransform();
			...
			//szprychy
	        for (int i = 0; i < (N/2); i++ ) {
	        	Shape line = new Line2D.Float((float)Math.sin(2*Math.PI/N*i)*2, (float)Math.cos(2*Math.PI/N*i)*2, (float)Math.sin(2*Math.PI/N*(i+6))*2, (float)Math.cos(2*Math.PI/N*(i+6))*2);
	        	g2.draw(line);
	        }
			g2.setColor(saveColor);
			g2.setTransform(saveTransform);	
